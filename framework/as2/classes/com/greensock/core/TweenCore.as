...
class com.greensock.core.TweenCore {
		/** @private **/
		public static var version:Number = 1.64;
		
		/** @private **/
		private static var _classInitted:Boolean;
		
		/** @private Delay in seconds (or frames for frames-based TweenCores) **/
		...
		public var initted:Boolean; 
		 /** Every tween belongs to one timeline. By default, it uses the TweenLite.rootTimeline (or TweenLite.rootFramesTimeline for frames-based TweenCores). **/
		public var timeline:SimpleTimeline;
		/** @private Start time in seconds (or frames for frames-based TweenCores), according to its position on its parent timeline **/
		...
		public function TweenCore(duration:Number, vars:Object) {
			this.vars = vars || {};
			...
			if (!_classInitted) {
				if (TweenLite.rootFrame == undefined && TweenLite.initClass != undefined) { //Flash 6 instantiates the classes in a very odd order, so we need to check to make sure initClass exists first to work around the problem.
					TweenLite.initClass();
					_classInitted = true;
				} else {
					return;
			...
			var tl:SimpleTimeline = (this.vars.timeline instanceof SimpleTimeline) ? this.vars.timeline : (this.vars.useFrames == true) ? TweenLite.rootFramesTimeline : TweenLite.rootTimeline;
		...
		/** Starts playing forwards from the current position. (essentially unpauses and makes sure that it is not reversed) **/
		 ...
		 */
		public function restart(includeDelay:Boolean, suppressEvents:Boolean):Void {
			this.reversed = false;
			this.paused = false;
		 ...
		 */
		public function reverse(forceResume:Boolean):Void {
			this.reversed = true;
		...
		/**
		 * @private
		 * Renders the TweenCore at a particular time (or frame number for frames-based tweens). 
		 * The time is based simply on the overall duration. For example, if a TweenCore's duration
		 * is 3, renderTime(1.5) would render it at the halfway finished point.
		 * 
		 * @param time time (or frame number for frames-based TweenCores) to render.
		 ...
		 * 
		 * @param skipRender to skip rendering the final state of the tween, set skipRender to true (it's false by default). 
		 * @param suppressEvents If true, no events or callbacks will be triggered for this render (like onComplete, onUpdate, onReverseComplete, etc.)
		 */
		public function complete(skipRender:Boolean, suppressEvents:Boolean):Void {
			if (!skipRender) {
				renderTime(this.totalDuration, suppressEvents, false); //forces the final render
		 ...
		 * have a TweenMax(mc, 1, {_x:100, _y:100}) that ran when mc._x and mc._y were initially at 0, but now mc._x 
		 * and mc._y are 200 and you want them tween to 100 again, you could simply invalidate() the tween and 
		 ...
		 * 
		 * @param enabled Enabled state of the TweenCore
		 * @param ignoreTimeline By default, the TweenCore will remove itself from its timeline when it is disabled, and add itself when it is enabled, but this parameter allows you to override that behavior.
		...
		/** Kills the TweenCore, stopping it immediately. **/
		 ...
		 */
		private function setDirtyCache(includeSelf:Boolean):Void {
			var tween:TweenCore = (includeSelf != false) ? this : this.timeline;
			while (tween) {
		 ...
		 * @param suppressEvents If true, no events or callbacks will be triggered for this render (like onComplete, onUpdate, onReverseComplete, etc.)
		 **/
		private function setTotalTime(time:Number, suppressEvents:Boolean):Void {
			if (this.timeline) {
...
//---- GETTERS / SETTERS ------------------------------------------------------------
		...
		/** 
		 * Length of time in seconds (or frames for frames-based tweens) before the tween should begin. 
