 ...
 * 		  the halfway point, set myTimeline.currentProgress = 0.5. </li>
 * 		  
 * 		<li> tween the <code>totalTime</code> or <code>currentProgress</code> property to fastforward/rewind the timeline. You could 
 ...
 * 		  
 * 		<li> If you need even more features like repeat, repeatDelay, yoyo, currentLabel, getLabelAfter(), getLabelBefore(),
 ...
 * 	
 * 		var myTimeline:TimelineLite = new TimelineLite({tweens:[new TweenLite(mc1, 1, {_y:"100"}), TweenMax.to(mc2, 1, {tint:0xFF0000}), new TweenLite(mc3, 1, {_y:"120"})], align:TweenAlign.SEQUENCE, onComplete:myFunction});</code><br /><br />
 * 	
...
class com.greensock.TimelineLite extends SimpleTimeline {
		/** @private **/
		public static var version:Number = 1.65;
		/** @private **/
		...
		public function insert(tween:TweenCore, timeOrLabel:Object):TweenCore {
			...
			if (typeof(timeOrLabel) == "string") {
				if (_labels[timeOrLabel] == undefined) {
					addLabel(String(timeOrLabel), this.duration);
				}	
		...
		public function append(tween:TweenCore, offset:Number):TweenCore {
			return insert(tween, this.duration + (Number(offset) || 0));
		}
		...
		public function insertMultiple(tweens:Array, timeOrLabel:Object, align:String, stagger:Number):Array {
			...
			var i:Number, tween:TweenCore, curTime:Number = Number(timeOrLabel) || 0, l:Number = tweens.length;
			if (typeof(timeOrLabel) == "string") {
				if (_labels[timeOrLabel] == undefined) {
					addLabel(String(timeOrLabel), this.duration);
				}
		...
		public function appendMultiple(tweens:Array, offset:Number, align:String, stagger:Number):Array {
			return insertMultiple(tweens, this.duration + (Number(offset) || 0), align, stagger);
		}
		...
		public function gotoAndPlay(timeOrLabel:Object, suppressEvents:Boolean):Void {
			setTotalTime(parseTimeOrLabel(timeOrLabel), Boolean(suppressEvents != false));
			this.play();
		}
		...
		public function hasPausedChild():Boolean {
			...
			while (tween) {
				if (tween.cachedPaused || ((tween instanceof TimelineLite) && TimelineLite(tween).hasPausedChild())) {
					return true;
				}
		...
		public function getChildren(nested:Boolean, tweens:Boolean, timelines:Boolean, ignoreBeforeTime:Number):Array {
			...
			var a:Array = [], tween:TweenCore =  (this.gc) ? _endCaps[0] : _firstChild, cnt:Number = 0;
			while (tween) {
		...
		public function getTweensOf(target:Object, nested:Boolean):Array {
			...
			var tweens:Array = getChildren(nested, true, false), a:Array = [], i:Number;
			var l:Number = tweens.length;
			var cnt:Number = 0;
			for (i = 0; i < l; i++) {
...
//---- GETTERS / SETTERS -------------------------------------------------------------------------------------------------------
		...
		public function get totalDuration():Number {
			if (this.cacheIsDirty) {
				var max:Number = 0, end:Number, tween:TweenCore = (this.gc) ? _endCaps[0] : _firstChild, prevStart:Number = -999999999999, next:TweenCore;
				while (tween) {
