package com.senocular.utils {
	...
	public class SWFReader {
		...
		// constants
		private const GET_RECT_DATA_SIZE:int	= 5;
		private const TWIPS_TO_PIXELS:Number	= 1/20; // 20 twips in a pixel
		private const COLOR_R_SHIFT:int			= 16; // R in RGB
		private const COLOR_G_SHIFT:int			= 8;  // G in RGB
		...
		private const TAG_ID_EOF:int			= 0; // recognized SWF tags
		private const TAG_ID_BG_COLOR:int		= 9;
		private const TAG_ID_PROTECTED:int		= 24;
		private const TAG_ID_DEBUGGER1:int		= 58;
		private const TAG_ID_DEBUGGER2:int		= 64;
		private const TAG_ID_SCRIPT_LIMITS:int	= 65;
		private const TAG_ID_FILE_ATTS:int		= 69;
		private const TAG_ID_META:int			= 77;
		...
		private function readRect():Rectangle {
			...
			var dataSize:uint	= readBits(GET_RECT_DATA_SIZE);
			rect.left			= readBits(dataSize)*TWIPS_TO_PIXELS;
			rect.right			= readBits(dataSize)*TWIPS_TO_PIXELS;
			rect.top			= readBits(dataSize)*TWIPS_TO_PIXELS;
			rect.bottom			= readBits(dataSize)*TWIPS_TO_PIXELS;
			return rect;
		}
		...
		private function readTag():Boolean {
			...
			var tag:Object = {
				id:		tagHeader >> TAG_HEADER_ID_BITS,
				length:	tagHeader & TAG_HEADER_MAX_SHORT
			};
			
			// if a long tag, the tag length will be
			// set to its maximum. If so, set
			// the tag length to the long length
			if (tag.length == TAG_HEADER_MAX_SHORT) {
				tag.length = bytes.readUnsignedInt();
			}
			...
			var nextTagPosition:int = bytes.position + tag.length;
			
			// read the data in the tag (if supported)
			var moreTags:Boolean = readTagData(tag);
		 ...
		 * true if more tags should be present in the file.
		 */
		private function readTagData(tag:Object):Boolean {
			...
			if (tagCallback != null) {
				...
				tagBytes.writeBytes(bytes, bytes.position, tag.length);
				tagBytes.position = 0;
				tagCallback(tag.id, tagBytes);
			}
			...
			switch (tag.id) {
				...
				case TAG_ID_BG_COLOR:
					...
					if (!bgColorFound) {
						...
						backgroundColor = (bytes.readUnsignedByte() << COLOR_R_SHIFT)
										| (bytes.readUnsignedByte() << COLOR_G_SHIFT)
										|  bytes.readUnsignedByte(); //      B no shift
					}
					break;
